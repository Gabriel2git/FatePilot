    ...
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 overflow-y-auto max-h-[calc(100vh-100px)]">
        ...
        {PERSONA_CONFIGS.map((config) => {
          ...
          return (
            <div
              ...
              className={`
                relative p-4 sm:p-6 rounded-2xl border-2 transition-all duration-300 cursor-pointer
            ...
            >
              {/* 选中标记 */}
              {isSelected && (
                <div className="absolute top-3 right-3 sm:top-4 sm:right-4 w-5 h-5 sm:w-6 sm:h-6 bg-purple-600 rounded-full flex items-center justify-center shadow-md">
              ...
              <h3 className={`text-base sm:text-lg mb-1 text-gray-900 dark:text-gray-100 ${isSelected ? 'font-bold' : 'font-semibold'}`}>
                {config.name}
              </h3>

              {/* 英文标题 - 统一灰色 */}
              <p className="text-xs mb-2 sm:mb-3 font-medium text-gray-500 dark:text-gray-400">
                {config.title}
              </p>
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4 sm:p-6 text-center">
        ...
        <button
          ...
          className="w-full sm:w-auto px-6 sm:px-8 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-medium rounded-lg shadow-lg hover:from-purple-700 hover:to-blue-700 hover:shadow-xl transition-all duration-200"
        >
          确认选择并进入
        </button>
      </div>
